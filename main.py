...
if __name__=="__main__":
    ...
    for iter in range(0, numiter):
        ...
        properties = stormpy.parse_properties(
            autonomous_agent.formula, prism_program)
        model = stormpy.build_sparse_model(prism_program)

        result = stormpy.model_checking(
            model, properties[0],  extract_scheduler=True)
        ...
        path_human = os.path.join(
            analyzer2.model_path, 'final_combined_model.nm')
        prism_program_human = stormpy.parse_prism_program(path_human)
        properties_human = stormpy.parse_properties(
            human_agent.formula, prism_program_human)

        model_human = stormpy.build_sparse_model(prism_program_human)
        result_human = stormpy.model_checking(
            model_human, properties_human[0],  extract_scheduler=True)
        ...
        Agent1_pol = autonomous_agent.get_policies(
            analyzer, model, result, ['go1', 'stop1'])

        # with open('pol.txt', 'w') as convert_file:
        #     convert_file.write(str(Agent1_pol))

        Agent2_pol = human_agent.get_policies(
            analyzer2, model_human, result_human, ['go2', 'stop2'])

        # create complete model applying policies ...
        if env_model_type == 'original':
            true_env_autonomous = autonomous_agent.get_environment_original()
        else:
        ...
        model = stormpy.build_sparse_model(prism_program)
        result = stormpy.model_checking(
            model, properties[0], only_initial_states=False, extract_scheduler=True)
        initial_state = model.initial_states[0]
        ...
        data_out.update(
            {(belief_model_prob, belief_model_prob_human): original_model_prob})
